...
class CHAST(nn.Module):
    ...
    def __init__(
        ...
        num_heads: int = 4,
        dropout: float = 0.1,
        kernel_size: int = 3,  # !!!!not mentioned in the paper but it's the default value in this codebase
    ):
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            nn.Linear(embed_dim, embed_dim * 2),  # embed_dim * 2 is not mentioned in the paper 
                                                  # but it's the default value in this codebase
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        h = self.act(self.conv2(h))  # (B, 2, K, L)
